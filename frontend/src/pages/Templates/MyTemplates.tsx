...
import { Plus, Store, Zap, Eye, EyeOff, Trash2, Loader2 } from 'lucide-react'
...
export default function MyTemplates({ templates, loading, onUse, onRefetch, purchased }: Props) {
  ...
  const handleTogglePublish = async (tmpl: Template) => {
    ...
    try {
      if (tmpl.is_published) {
        ...
        toast.success(t('templates.my.hidden') + ' ✓')
      } else {
        await templatesApi.publish(tmpl.id)
        toast.success(t('templates.my.published') + ' ✓')
      }
      onRefetch()
    } catch {
  ...
  const handleDelete = async (tmpl: Template) => {
    ...
    try {
      ...
      toast.success(tmpl.title + " o'chirildi")
      onRefetch()
    } catch {
      toast.error(t('common.error'))
    } finally {
      ...
      <div className="text-center py-14 bg-tg-card rounded-[20px] flex flex-col items-center gap-3">
        ...
        <div>
          <p className="text-white text-[15px] font-semibold m-0 mb-1">
            {purchased ? t('templates.purchased.empty') : t('templates.my.empty')}
          </p>
          <p className="text-tg-label text-[13px] m-0">
            {purchased ? t('templates.purchased.empty_desc') : t('templates.my.empty_desc')}
          </p>
        </div>
        {!purchased && (
          <button
          ...
          >
            <Plus size={14} />
            {t('templates.my.create')}
          </button>
        )}
        {showPublish && (
          <PublishTemplateModal
            onClose={() => setShowPublish(false)}
            onCreated={() => { setShowPublish(false); onRefetch() }}
          />
        )}
      </div>
    )
              ...
              <div className="flex items-center gap-2 mb-0.5">
                ...
                {!purchased && (
                  <span className={`text-[9px] font-bold px-1.5 py-[2px] rounded-full shrink-0 ${
                    ...
                    {tmpl.is_published ? t('templates.my.published') : t('templates.my.hidden')}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-2 text-[11px] text-tg-muted">
                {tmpl.category && <span>{tmpl.category}</span>}
                {tmpl.uses_count > 0 && <span>· {tmpl.uses_count}↓</span>}
                {tmpl.price_stars === 0
                  ? <span className="text-green-400">· {t('templates.free')}</span>
                  : <span className="text-yellow-400">· ⭐ {tmpl.price_stars}</span>
                }
                {tmpl.review_count > 0 && (
                  <span>· ★ {tmpl.avg_rating.toFixed(1)}</span>
                )}
              </div>
            </div>
              ...
              {!purchased && (
                <>
                  <button
                    ...
                    disabled={busy}
                    title={tmpl.is_published ? t('templates.my.unpublish') : t('templates.my.publish')}
